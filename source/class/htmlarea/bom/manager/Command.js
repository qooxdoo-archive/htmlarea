 ...
 */
qx.Class.define("htmlarea.bom.manager.Command",
{
   ...
   * Constructor
   *
   * @param editorInstance {htmlarea.bom.HtmlArea} editor instance
   */
  ...
  members :
  {
    ...
    __paragraphMissing : function()
    {
      ...
      if (focusNode)
      {
        if (Node.isText(focusNode))
        {
          ...
          {
            if (Node.isNodeName(parents[i], "p") || htmlarea.bom.HtmlArea.isHeadlineNode(parents[i]))
            {
     ...
     * *********************************************************** */
     ...
     __insertHtml : qx.core.Environment.select("engine.name",
     {
       "mshtml" : function(value, commandObject)
       {
         ...
         if (value == htmlarea.bom.HtmlArea.simpleLinebreak) {
      ...
      __hideSuperfluousParagraph : qx.core.Environment.select("engine.name",
      {
        "gecko" : function()
        {
              ...
              prevSiblingId == nextSiblingId)
          {
            ...
            var htmlToInsert = htmlarea.bom.HtmlArea.EMPTY_DIV;
     ...
     __insertBrOnLinebreak : qx.core.Environment.select("engine.name",
     {
       "mshtml" : function()
       {
         ...
         {
           rng.pasteHTML(htmlarea.bom.HtmlArea.simpleLinebreak);
           rng.collapse(false);
           rng.select();
     ...
     */
     ...
     __setBackgroundImage : function(value, commandObject)
     {
       ...
       var Command = htmlarea.bom.manager.Command;
